...
在平台上调试请求 `https://gds.store-api-stg.middlelayer.***.net/store/testSendMsg?fohNos=88802`，执行失败；同一地址在终端 `curl` 正常。用户怀疑平台不支持 HTTPS。
...
1. 该 STG 域名解析到内网 IP（10.94.72.27），证书由 `*** Corporate Issuing CA 07`（企业内部 CA，向上链到 `*** Root CA`）签发，非公共 CA。
...
1. 从系统钥匙串导出企业 CA（Root + 各 Issuing CA，共 10 张）到 `.dev-certs/***-ca.pem`（gitignore，不入库）。钥匙串里按 CA 名筛选，名字不写进仓库，从环境变量取：

   ```bash
   CORP_CA_KEYCHAIN_NAME=<企业 CA 名>
   security find-certificate -a -c "$CORP_CA_KEYCHAIN_NAME" -p /Library/Keychains/System.keychain \
