...
> 站内助手四个新阶段插在 P5 之后，**原 P6（性能/插件/版本）顺延为 P10**（**2026-09-18 插件
> 机制整项砍除，现为「性能/版本」；14.1 性能拆三批 P10-1~P10-3——**P10-1 已实现
> （2026-09-19，迁移 065：大响应体转存对象存储，见该文件 14.1 实现状态）**、**P10-2 已实现
> （2026-09-20，迁移 066：`executions` 按季分区 + 冷分区 DETACH 归档 + 清理时同步删对象，
> 同上）**、**P10-3 已实现（2026-09-20，零迁移零新依赖：进程内 stats 缓存换 Redis 共享缓存
> `lib/queryCache.ts` + 扩到 /dashboard、GET /projects + 写失效钩子，见该文件 14.1 实现状态）**，14.3 版本历史拆三批
> P10-4~P10-6，14.5 前端列表首屏闪空态修复 P10-7、14.6 顶层操作区不随内容滚动 P10-8**，
> 见扩编文档十四章）。五个阶段的
> 范围、边界、迁移（051/052 + P8 054–056 / P9 057——053 已被 P7 状态评审占用，2026-09-09
> 顺延；2026-09-11 复核 P8 实占 054–056，P9 顺延 057、P11 顺延 058）、路由、批次与验收门槛见
> **`DEVELOPMENT_PLAN_P6-P14.md`**（章节映射：该文件十~十四章 = 本计划的 P6~P10；
> P10 于 2026-09-05 自本计划十四章并入该文件）。
> **P11 已立项（2026-09-07；2026-09-17 增补边界 11「触发人可见」，同日 P11-1 迁移 064、
> P11-2 内容资产、P11-3 调度/告警、P11-4 MCP/导入、P11-5 读侧、P11-6 触发人、P11-7 前端
> 归属列、P11-8 触发人展示、P11-9 审计动作/挂载与 P11-10 审计读侧/前端已实现）**：
> 资产归属——14 张核心表补 created_by / updated_by + CRUD 接入 audit_logs，
> 外加执行/报告记录显示「谁触发」（`executions` 补 `triggered_by`），见该文件十六章与本计划十五章。
> **P12 已立项（2026-09-14，尚未实现）**：个人中心 + 用户级 MCP Token——token
> 归人、绑定归项目（scope 放绑定上）、工具面 projectId 参数化、read 补活体复查
> （改写 P5 9.1/9.3 的项目级 token 口径，见 9.0 修订指针）；全局层 `/me` 页集齐
> 改密 / 通知查看全部 / MCP Token / agent 凭据挂点（P9-4b 悬置的「个人信息界面
> 另行规划」在此兑现）。见该文件十八章与本计划十六章。
> **P13 已立项（2026-09-16 立项；2026-09-21 改题，尚未实现）**：**原「新手教程」
> （P9-7 教程骨架 + 主线 5 步 + 双 i18n）整项砍除**，P13 改为——① `/me`
> 个人中心五 tab（基本信息 / 通知 / 我的申请 / MCP Token / agent 凭据）整体并入
> 站内助手抽屉、删 `/me` 路由 + 删基本信息里的改密表单；② 助手卡片空态问候改
> `HI, {{用户名}}`；③ 新增应用内 `/docs` 功能说明文档（头像旁 `_blank` 入口、
> 左标题目录 + 右正文、双 i18n）。**2026-09-21 二轮反馈（走查，已实现）**：个人中心
> 删「通知」分区（通知回到抽屉 `notifications` 视图）+ MCP Token 平铺成两张表且 tab
> 条常驻 + **新增 MCP 绑定许可申请**（成员申请角色上限之外的档位、通知 project_admin、
> 批准即授权；迁移 070 + 五条 REST + `/mcp` 闸门放宽——这一项带后端）。见该文件
> 十九章与本计划十七章。
> **P14 已立项（2026-09-16，尚未实现）**：项目可见性两层 + 权限申请审批流——
> 13.7 后置增补（2026-09-07 提出）转正排期：项目身份（名称/描述）对全部登录用户
> 可见、指标仍只对成员下发；非成员 URL 直达渲染「无权限 + 申请入口」落地页而非裸
> 403；新表 `access_requests` 审批流（批准 = 既有成员 upsert）+ 站内信两路触达
> （申请 → project_admin，结果 → 申请人）。见该文件二十章与本计划十八章。
...
> 2026-09-09 实现**（详见 `DEVELOPMENT_PLAN_P6-P14.md` 十二章 12.6 末尾实现状态）；
...
>
> **P9（站内助手）范围修订（2026-09-11 用户确认）**：账号操作（系统
> 设置快捷入口 / 登出 / 本人改密）并入助手，全走本地通道不经外部 agent；助手代理
> provider 化——`assistant_providers` 系统级表（多 provider + AES-GCM 加密 Key +
> JSON 导入导出 + 测试连接），adapter 契约 + 注册表（形制照 `lib/adapters`），v1
> 仅 `nuwax` 一个 adapter、**只接 agent 平台不接裸 LLM**（资格线：建会话 + 流式 +
> 历史）；多轮上下文与历史全由 agent 方持有、平台只代理展示；新增 13.6「agent 方
> 接入要求与对接契约」（含 MCP 工具面接线、`apitest-proposal` 草稿格式、elicitation/
> MRTR 三层职责拆解与实测方法）。迁移顺延 **057**（P11 顺延 058）。详见
> `DEVELOPMENT_PLAN_P6-P14.md` 十三章 13.0 修订与 13.4 批次重排。
> **P9-1（迁移 057 + 057b + inbox + 铃铛 + 通知列表页 + 五个生产源）已于
> 2026-09-11 实现**（状态见该文件 13.4.1）。收件口径（同日用户确认修订）：
> alert / suite_result / ci_result **谁触发发给谁**（手动 / MCP execute / 立即
> 运行带人；定时 / Webhook 非人触发不发站内）；member_change 全员、plan_assign
> 单人。
> **P9-2（代理底座 provider 化：adapter 契约 + 注册表 + nuwax 实现 + 助手五条
> 路由 SSE + 事件归一）已于 2026-09-11 实现**（状态见该文件 13.4.2）：stg 真实
> 平台联调**正常流式已验证**（agentId 更正为 26 后 token 逐段 + done 终态；
> 两处协议实况——body 带 conversationId、FINAL_RESULT success:false 映射 error
> ——与更新后的官方文档口径一致）+ apitest-e2e-python 八用例 8/8 通过。13.6
> 两项问询已答复（均不支持，proposal-first 收口维持）；功能请求见
> `智能体平台-MCP支持需求.md`（elicitation 转述 + 会话级凭据），MCP 接线待
> 平台部署到 Nuwax 可达网段后进行。
> **P9-3（provider 管理：CRUD + 加密 Key + JSON 导入导出 + 测试连接 +
> `assistant_enabled` 闸门 + 无默认 provider 引导）已于 2026-09-11 实现**（状态
> 见该文件 13.4.3）：后端九条系统级路由（类型目录注册表驱动 / Key 三态补丁 /
> 能力面只收窄 / set-default 事务保唯一 / 测试连接归一事件 SSE 回显）+ 前端系统
> 管理「助手 provider」tab；项目侧新增「站内助手」页挂 assistantEnabled 开关与
> 未配置引导（计划未指定归宿的落点决策，见 13.4.3）；e2e 种子脚本就此退休。
> 零新迁移（057 已立全部结构）。
>
...
```
P0 ──→ P1 ──→ P2 ──→ P3 ──→ P4 ──→ P4.5 ──→ P5 ──→ P6 ──→ P7 ──→ P8 ──→ P9 ──→ P10 ──→ P11 ──→ P12 ──→ P13 ──→ P14
MVP    流程    数据源  套件调度 仓库    Runner   MCP    用户   测试   数据   站内   性能    资产    个人中心  卡片    权限
       编排    节点补全 报告告警 用例                     权限   管理   统计   助手   版本    归属    ·用户级  收口    申请
       断言    套件    趋势                                      （2026-09-04 新增，P6–P10 详见
                                                                  DEVELOPMENT_PLAN_P6-P14.md；
                                                                  原 P6 性能/插件/版本顺延为 P10
                                                                  （2026-09-18 插件机制砍除，
                                                                  现为性能/版本）；
                                                                  P11 于 2026-09-07 立项、P12 于
                                                                  2026-09-14 立项、P13 于 2026-09-16
                                                                  立项（2026-09-21 改题：教程砍除，
                                                                  改为个人中心并入助手卡片 +
                                                                  /docs 文档；2026-09-21 二轮反馈
                                                                  增 MCP 绑定许可申请，带迁移 070）、
                                                                  P14 于 2026-09-16 立项（13.7 转正），
                                                                  同文件十六 / 十八 / 十九 / 二十章）
```
...
1. `[已实现]` 全局层：新增数据看板、项目管理、系统管理；登录默认进入数据看板。
   ...
   - `[已实现]` **项目壳侧栏视觉改版（2026-09-19 用户确认「V3」方向）**：上一条「项目壳侧栏形态不变」到此更新——保留手风琴分组（六组十六项，收着才不出常驻滚动条，这也是否决「全项平铺」V1 的原因），但把组头三件套（caret + 组图标 + 大写标题）重排：组图标与组名领读、caret 移到组头右缘并调淡（`--line-strong`）。选中态在**竖向侧栏**（展开 rail / 收起 rail / ≤940px 顶栏模式的竖向态）去掉 `--accent-soft` 药丸底，改中性 `--hover` 底 + 左缘 accent tick + 图标 accent——hover 与选中共用一层底色，强调色只落在 tick 与图标上，用来把「当前页」与「刚划过」分开；全局层 `.topbar-nav` 的选中标记（accent-soft）不动。收起 56px rail 的组分隔由原来的匿名 4px 短横改成**组图标居中**当分隔（同一图标展开领读组名、收起即组标记，不展开也能认组），组名文字 `font-size: 0` 让位、留 DOM 给读屏并挂 `title`。改动落 `ProjectShell.tsx`（组头 markup）+ `design-system.css`（`.nav-section` caret、`.sidebar .nav-item.is-active`、收起态 `.nav-section`）+ `theme-preview.tsx`（参考页同步）。未跑 `pnpm check` / `pnpm build`、未做浏览器走查（验证由用户完成）。
...
插件（原计划 P10 14.2，**2026-09-18 已整项砍除**）解决的是「平台不知道你要干什么」的扩展点。
这里不是：**「一次执行该带哪些头」
...
- **硬约束：平台永不代理大文件流**。Runner 直传对象存储，浏览器下载也走 `presignGet` 出来的
  ...
  （2026-09-11 修订：收窄为「上传不代理；下载侧允许流式转发」，未排期，条目在
  `DEVELOPMENT_PLAN_P6-P14.md` 十七。）
...
> **Token 形状已被 P12 改写（2026-09-14 立项，尚未实现）**：`mcp_tokens` 升级为
> **用户级**——token 归人、`mcp_token_projects` 绑定表归项目、scope 放绑定上、
> 工具面加必填 `projectId` 参数（集中注册包装注入，非逐个改声明）、read 补活体
> 复查、闸门重排五道。本节及 9.1 /
> 9.3 描述的项目级 token（`project_id` / scope 挂 token 行）以
> `DEVELOPMENT_PLAN_P6-P14.md` 十八章 18.0 边界为准；`/mcp` 端点、无状态模型、
> 前缀收窄 + scrypt、MRTR 确认、读时脱敏等机制性决策不变。
...
> **已立项为 P9（2026-09-04）**：范围、边界与批次见 `DEVELOPMENT_PLAN_P6-P14.md` 十三章。
> 上游能力核对（`智能体平台-第三方接入接口文档.md`）与 L3 定案（proposal-first）以
> 该文件为准；本节保留为方向性说明。**新手教程曾于 2026-09-16 自 P9-7 移出为 P13，
> 后于 2026-09-21 整项砍除**（P13 改题为个人中心并入助手卡片 + `/docs` 文档，见
> 该文件十九章）。
...
定型再填。（**曾立项为 P13**，2026-09-16 自 P9-7 移出，**后于 2026-09-21 整项砍除**
——用户判定一条主线教程价值不足；P13 已改题为个人中心并入助手卡片 + `/docs` 文档，
见 `DEVELOPMENT_PLAN_P6-P14.md` 十九章。）
...
## 十四、P10 — 性能、版本（3 周，已并入扩编文档；插件机制 2026-09-18 砍除）

> **本章已于 2026-09-05 整章移入 `DEVELOPMENT_PLAN_P6-P14.md` 十四章**（内容原样未改）。
> **2026-09-18 修订**：14.2 插件机制整项砍除（三个窄缺口用户明确全部不要）；14.1 性能拆为
> 三批 P10-1（大响应体口径变更 + 转存对象存储，迁移 065）/ P10-2（执行历史归档分区表，
> 迁移 066）/ P10-3（查询缓存补齐，零迁移）；14.3 版本历史展开为三批 P10-4（统一版本模型 +
> 资产接入，迁移 067）/ P10-5（版本列表 + 改动内容 diff 展示 + 回滚）/ P10-6（执行结果回放，
> 边界待确认）/ P10-7（前端列表首屏闪空态修复，零迁移纯前端）/ P10-8（顶层操作区不随内容
> 滚动，零迁移纯前端，改动面较大）；**八批实施顺序见该文件 14.4**（迁移线串行 065 → 066 →
> 067，零迁移批次并行收口；前端线 P10-7 → P10-8 内部串行）。范围、顺延背景与口径说明见该
> 文件十四章。
> **2026-09-20 修订**：P10-5 四类入口全接完（接口/用例/文本用例/环境）；**P10-6 重定义**为
> 「版本历史扩面到 6 类用户编写型资源（Mock/脚本/流程/套件/数据源含 SQL 定义/仓库任务）+
> 变更历史（ChangeHistoryDrawer）资源详情入口全下线」，**原「执行结果回放」整项删除**（价值
> 不足）；新增迁移 068 仅扩 `asset_versions.asset_type` CHECK 枚举。详见该文件 14.3 的 P10-6
> 小节与 14.4 执行序列。

---

## 十五、P11 — 资产归属（谁创建 / 谁更新，约 1.5 周，已并入扩编文档）

> **本章为指针章（2026-09-07 立项）。** 完整范围、边界 11 项、迁移示意、改动量摸底
> 与分批交付（第一轮 P11-1 ~ P11-8 列级归属 + 触发人可见 / 第二轮 P11-9 ~ P11-10
> 审计级归属，实施顺序见 16.6）见
> `DEVELOPMENT_PLAN_P6-P14.md` 十六章。一句话范围：14 张 P0–P3 核心表补
> `created_by` / `updated_by`（列级）+ 核心资源 CRUD 接入 `audit_logs`
> （审计级），回答「这个接口谁建的、上次谁改的」；**另加触发人可见（2026-09-17 增补，
> 边界 11）**——执行/报告记录显示「谁触发的」，`executions` 补 `triggered_by`。
>
> **实现状态（2026-09-17）**：P11-1 迁移 `064_p11_ownership.sql`（14 表
> `created_by` / `updated_by` + 14 索引 + `executions.triggered_by`）、P11-2 内容资产 REST
> 写路径、P11-3 调度/告警 REST 写路径、P11-4 MCP 与导入写路径（含 `lib/caseSync.ts` /
> `lib/suiteMembers.ts` 两处尾巴与系统路径复核）、P11-5 读侧（16 个 mapper +
> `createdByEmail` / `updatedByEmail` + 13 个列表查询 `LEFT JOIN users`）、P11-6 触发人
> （`EnqueueInput.triggeredBy` + 子执行继承 + `triggeredByEmail` 读侧）、P11-7 前端归属列
> （`api.ts` `Ownership` + 12 个列表位 + `OwnerCell` + i18n）、P11-8 触发人展示
> （`TriggeredByEmail` + `lib/trigger.ts` + 执行记录三 tab 与报告侧的触发列）、P11-9 审计动作
> 与挂载（`AUDIT_ACTIONS` 42 动作 + `auditFields()` + 全写路径与 MCP 写工具的
> fire-and-forget 挂载，detail 只记字段名）、P11-10 审计读侧与前端（`requireDeveloper` 把项目
> 审计路由从 project_admin 放宽到 developer、`action`/`targetType`/`targetId` 过滤 + 分页、
> 项目配置 → 审计日志页、资源详情「变更历史」抽屉，不做 diff）均已在同日实现。
> 迁移未在本地库执行、未跑 `pnpm check` / `pnpm build`、
> 未做浏览器走查（验证由用户完成）。实施期的发现与限制（`environments` 无时间列、边界 3 对
> `caseSync.ts` 的误记更正、`test_suites.notify_config` 级联摘除、SpecCases 只有创建人、
> 批量触发人由成员行派生、CaseBox 是卡片列表、执行记录三 tab 原先没有触发列）见
> `DEVELOPMENT_PLAN_P6-P14.md` 16.6 实现状态与边界 3。

---

## 十六、P12 — 个人中心 + 用户级 MCP Token（约 2 周，已并入扩编文档）

> **本章为指针章（2026-09-14 立项）。** 完整范围、边界 12 项、迁移示意（063，原
> 060 顺延）、
> 改动面摸底、API 面与分批交付（P12-1 后端整批 / P12-2 前端整批）见
> `DEVELOPMENT_PLAN_P6-P14.md` 十八章。一句话范围：MCP Token 升级为**用户级 +
> 项目绑定**（scope 放绑定上、工具面 projectId 参数化、read 补活体复查），
> 外加全局层 `/me` 个人中心页，集齐改密 / 通知查看全部（`/notifications` 迁入）/
> MCP Token 管理 / agent 凭据挂点（P9-4b 的「个人信息界面另行规划」在此兑现）。
>
> **实现状态（2026-09-16）**：P12-1 后端整批与 P12-2 前端整批均已实现，**尚未验收**。
> 前端落点：`MePage.tsx`（`/me` 四 tab，通知 tab = `/me/notifications`）+
> `McpPage.tsx` 降级为项目侧只读绑定。**账号区（个人中心 + 登出）不在两个壳的顶栏，
> 而在站内助手的抽屉头**（2026-09-16 用户反馈修订）——旧 `/notifications` 路由与
> 项目侧签发入口均已删除，18.5 决定 2 记录了完整落点。

---

## 十七、P13 — 个人中心并入助手卡片 + 卡内问候 + 功能说明文档（已并入扩编文档）

> **本章为指针章（2026-09-16 立项；2026-09-21 改题）。** 完整范围、边界与
> 验收门槛见 `DEVELOPMENT_PLAN_P6-P14.md` 十九章。**原「新手教程」（P9-7
> 教程骨架 + 主线 5 步 + 双 i18n）2026-09-21 用户确认整项砍除**，v1.0 留档不实现。
> 一句话范围（改题后）：① `/me` 个人中心五 tab 整体并入站内助手抽屉
> （`AssistantDock`）、删 `/me` 路由 + 删基本信息里的改密表单（抽屉内既有改密视图
> 保留）+ 旧回链改跳抽屉；② 助手卡片空态问候改 `HI, {{用户名}}`（双 i18n）；
> ③ 新增应用内 `/docs` 功能说明文档（头像旁 `_blank` 入口、左标题目录 + 右正文、
> 双 i18n，初稿本方起草）。
>
> **2026-09-21 二轮反馈（走查验收，已实现）**：① 个人中心删「通知」分区（通知回到
> 抽屉 `notifications` 视图，恢复 bell-item 列表样式，`NotificationsPage` 删除）；
> ② MCP Token 去卡片、平铺成「Token / 绑定明细」两张表，me 视图返回键与 tab 条固定
> 不随正文滚动；③ **新增 MCP 绑定许可申请**（P13-4）：成员申请角色上限之外的
> write / execute 档位、站内通知 project_admin、批准行即授权（有效 scope = 角色上限
> ∪ 已批准申请，绑定校验与 `/mcp` 第 5 道闸门都读它）、不改成员角色。信息架构三项
> 仍零后端；第 ③ 项带**迁移 070 + 五条 REST**。详见该文件 19.0b / 19.5。

---

## 十八、P14 — 项目可见性两层 + 权限申请审批流（自 13.7 转正，已并入扩编文档）

> **本章为指针章（2026-09-16 立项）。** 完整范围、边界、迁移示意、路由、批次与
> 验收门槛见 `DEVELOPMENT_PLAN_P6-P14.md` 二十章。一句话范围：13.7 后置增补
> （2026-09-07 提出，原「记录取向不排期」）转正排期——项目身份（名称 / 描述）对
> 全部登录用户可见、指标仍只对成员计算与下发（看板聚合分母不含非成员项目）；非成员
...
| P7 | `exceljs` | `.xlsx` 固定模板解析（+计划导出的升级路径）；后端按需 `import()`，不进启动路径。P6/P8/P9/P10 零新增依赖——详见 `DEVELOPMENT_PLAN_P6-P14.md` 15.1 |
...
| M8     | P6      | 注册 + 成员与角色 + 三级权限 + 审计可读 | 邀请码注册即入项目；viewer 界面无写入口；成员变更可审计（`DEVELOPMENT_PLAN_P6-P14.md` 十章） |
...
| M11    | P9      | 站内助手 + 站内通知（教程 2026-09-16 移出至 P13 / M14） | 代理对话可用且 Key 不出服务端；proposal-first 闭环；通知小红点 + 弹窗（同文件十三章） |
| M7     | P10     | 性能 + 版本历史 + 前端体验（原 P6 顺延；插件机制 2026-09-18 砍除） | 支持 1000+ 并发执行；变更历史可查看改动内容；列表首屏不闪空态；顶层操作区不随内容滚动（同文件十四章） |
| M12    | P11     | 列级归属 + 触发人可见 + 资源 CRUD 全审计 | 任意核心资源可查「谁建/谁改」（列表直读 + 审计下钻两路）；执行/报告可查「谁触发」（手动一次后父执行与子执行同一触发人）；系统写入与定时/Webhook 显示「系统」；viewer 无审计读权（同文件十六章） |
| M13    | P12     | 个人中心 + 用户级 MCP Token | 一把 token 跨项目（A 写 B 读同一会话完成）；个人中心集齐改密/通知查看全部/MCP Token/agent 凭据挂点；项目侧绑定可见可踢；吊销/解绑/关闸/降权即时生效（同文件十八章） |
| M14    | P13     | 个人中心并入助手卡片 + 卡内问候 + `/docs` 文档 + MCP 绑定许可申请（2026-09-21 改题，原「新手教程」砍除；二轮反馈增第 3 项） | `/me` 路由删除且个人中心分区在助手卡片内功能完整、tab 条常驻；基本信息无改密表单（抽屉改密保留）；通知只有抽屉 `notifications` 视图一个落点；卡片空态 `HI, {{用户名}}` 双语；头像旁 `_blank` 打开 `/docs`，左目录 + 右正文双语；MCP 绑定许可申请闭环（成员申请 → project_admin 通知 → 批准即授权 → 个人中心可绑定）且不改成员角色；无 `/me` 死链（同文件十九章） |
| M15    | P14     | 项目可见性两层 + 权限申请审批流（自 13.7 转正） | 非成员可见项目身份（名称/描述）但任何指标不可见，看板聚合分母不含非成员项目；URL 直达渲染「无权限」落地页而非裸 403；申请→站内信通知 project_admin→批准（走既有成员 upsert）/拒绝→结果通知申请人全程闭环；同一 (project, user) 只有一条 pending（同文件二十章） |
