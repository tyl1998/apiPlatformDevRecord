...
> **P13 已立项（2026-09-16，尚未实现）**：新手教程——P9-7（教程骨架 + 主线 5 步
> + 双 i18n）自 P9 整项移出，P9 范围收窄为人物 + 聊天 + 站内通知。见该文件十九章
> 与本计划十七章。
...
MVP    流程    数据源  套件调度 仓库    Runner   MCP    用户   测试   数据   站内   性能    资产    个人中心  新手    权限
       编排    节点补全 报告告警 用例                     权限   管理   统计   助手   版本    归属    ·用户级  教程    申请
...
> 该文件为准；本节保留为方向性说明。**新手教程已于 2026-09-16 自 P9-7 移出为 P13**
> （见该文件十九章）。
...
定型再填。（**已立项为 P13**，2026-09-16 自 P9-7 移出，见
`DEVELOPMENT_PLAN_P6-P14.md` 十九章。）
...
## 十七、P13 — 新手教程（自 P9 移出，已并入扩编文档）

> **本章为指针章（2026-09-16 立项）。** 完整范围、边界与验收门槛见
> `DEVELOPMENT_PLAN_P6-P14.md` 十九章。一句话范围：P9-7「教程骨架 + 主线 5 步 +
> 双 i18n」自 P9 整项移出——数据驱动骨架（`{ route, selector, i18nKey }[]` JSON +
> `preferences.onboarding` 记完成态，零迁移）+ 一条 5 步主线（登录 → 建项目 →
> 建接口 → 跑一次 → 看报告）+ 侧边卡片 + `data-tour` 描边，双 i18n。
...
| M14    | P13     | 新手教程（自 P9-7 移出） | 主线教程 5 步全程无「目标元素找不到」的卡死（找不到自动跳步）；首登自动触发且完成态可记（`preferences.onboarding.mainDone`）；双 i18n（同文件十九章） |
