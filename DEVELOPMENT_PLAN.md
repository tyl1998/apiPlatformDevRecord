...
> created_by / updated_by + CRUD 接入 audit_logs，见该文件十六章与本计划十五章。
...
| M12    | P11     | 列级归属 + 资源 CRUD 全审计 | 任意核心资源可查「谁建/谁改」（列表直读 + 审计下钻两路）；系统写入显示「系统」；viewer 无审计读权（同文件十六章） |
